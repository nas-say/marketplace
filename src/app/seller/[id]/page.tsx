import { getUserById, getListingsBySeller, getBetaTests, formatPrice, getUsers } from "@/lib/data";
...
export function generateStaticParams() {
  return getUsers().map((user) => ({ id: user.id }));
}
...
export default async function SellerPage({ params }: SellerPageProps) {
  ...
  const betaTests = getBetaTests().filter((bt) => bt.creatorId === id);
  const activeBetaTests = betaTests.filter((bt) => bt.status !== "closed");
      ...
      <div className="grid grid-cols-1 gap-8 lg:grid-cols-[300px_1fr]">
        ...
        <aside className="lg:sticky lg:top-24 lg:self-start">
          <div className="rounded-lg border border-zinc-800 bg-zinc-900 p-6">
            ...
            <div className="space-y-2 text-sm text-zinc-500 mb-6">
              ...
              {seller.social.github && (
                <div className="flex items-center gap-2">
                  ...
                  <a
                    href={`https://github.com/${seller.social.github}`}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="hover:text-indigo-400"
                  >
            ...
            <div className="border-t border-zinc-800 pt-4 text-xs text-zinc-500 text-center">
              Member since{" "}
              {new Date(seller.stats.memberSince).toLocaleDateString("en-US", {
                month: "long",
                year: "numeric",
              })}
            </div>
        ...
        <div>
          ...
          <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 mb-8">
            <StatCard
              label="Total Sales"
              value={String(seller.stats.totalSales)}
              icon={<Star className="h-4 w-4" />}
            />
            <StatCard
              label="Total Earnings"
              value={formatPrice(seller.stats.totalEarnings)}
          ...
          {activeBetaTests.length > 0 && (
            <div>
              <h2 className="text-xl font-bold text-zinc-50 mb-4">
                Open Beta Tests
                <span className="ml-2 text-sm font-normal text-zinc-500">({activeBetaTests.length})</span>
              </h2>
                  ...
                  >
                    <div>
                      ...
                      <p className="text-xs text-zinc-500 mt-0.5">
                        {bt.spots.filled}/{bt.spots.total} spots filled · {bt.reward.description}
                      </p>
                    </div>
                    <Badge
                      className={
